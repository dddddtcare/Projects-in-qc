...
//----------------------------------------------------------------------
// Draw one snowman (at the origin)
//
//----------------------------------------------------------------------
void drawSnowman()
{
	...
	glutSolidSphere(0.05, 20, 20); // right eye
	glPopMatrix();
	glPushMatrix();
	glTranslatef(+0.05, 0.0, 0.0);
	glutSolidSphere(0.05, 20, 20); // left eye
	glPopMatrix();
	glPopMatrix();
	...
	// Draw a hat for snow man
	glColor3f(1.0, 0.5, 0.0);
	glPushMatrix();
	glTranslatef(0.0, 0.0, 1.99);	// translate to just above ground
	glScalef(1.0, 0.8, 0.1); // scale sphere into a flat pancake
	glutSolidSphere(0.55, 20, 20); // shadow same size as body
	glPopMatrix();

	glPushMatrix();
	glTranslatef(0.0, 0.0, 1.99);	// translate to just above ground
	glutSolidCone(0.20, 0.5, 10, 2); // draw cone
	glPopMatrix();

	// Drawa hands for snow man
	glColor3f(1.0, 0.5, 0.0); // color for hands
	glPushMatrix();
	glTranslatef(0.0, -0.18, 0.05); // lift hands to final position
	glPushMatrix();
	glRotatef(70.0, 0.0, 1.0, 0.0);
	glTranslatef(-0.8, 0.0, 0.95);

	glScalef(1.0, 0.8, 9); // scale a cube into a cuboid
	//glutSolidSphere(1, 20, 20); // right hand
	glutSolidCube(0.1);
...
void renderScene(void)
{
	...
	// Draw a moving snow man


	for (i = -3; i < 3; i++)
		for (j = -3; j < 3; j++) {
		glPushMatrix();
		glTranslatef(i*7.5, j*7.5, 0.9 + sin(moving));
		drawSnowman();
		glPopMatrix();
		}
...
void processNormalKeys(unsigned char key, int xx, int yy)
{
	...
	if (key == 'W' || key == 'w' && speed <= 5) {
		speed += 0.5;
		cout << "You increase the speed of jumping---Speed X " << speed << endl;
		glutPostRedisplay();

	}
	if ((key == 's' || key == 'S') && speed > 0) {
		speed += -0.5;
		cout << "You decrease the speed of jumping---Speed X " << speed << endl;
		glutPostRedisplay();
	}
}
...
- Press w/s to increase/decreas the speed of jumping\n\
- q or ESC to quit\n\
-----------------------------------------------------------------------\n");
	...
	glutCreateWindow("Dancing Snowmen");
